module.exports = function(api, users){
    ...
    api.listen(
        ...
        function(from, interface, message){
            users.findOneAsync({$and: [{name: message[2]}, {interface: message[1]}]}).then(function(doc){
                if (doc) {
                    return users.updateAsync({$and: [{name: message[2]}, {interface: message[1]}]}, {$set: {role: message[0]}});

                } else {
                    return users.insertAsync({"name":message[2],"interface":message[1],"role":message[0]});
                }
            }).then(function(){
    ...
    api.listen(
        ...
        'standard',
        function(from, interface, message, user){
            api.users.findOneAsync({$and: [{name: user}, {interface: interface}]}).then(function(doc){
                if(doc){
                    return doc.role;
                }

                return api.prefs.interfaces.findOneAsync({name: interface}).then(function(doc) {
                    if (doc.default_permission) {
                        return doc.default_permission;
                    }

                    return api.prefs.base.findOneAsync({name: 'default_permission'}).then(function(permission){
                ...
                for (var key in api.listeners) {
                    ...
                    listener = api.listeners[key].listener.replace(/(\((:<(.+?)>)*([\(].+?[\)])*(.+?)\))/g,
                    ...
                    displayList[api.listeners[key].module.displayname].push('        - Command: ' + listener);
                    displayList[api.listeners[key].module.displayname].push('        - Role needed: ' + api.listeners[key].role + '\n');
                }
