...
module.exports = function(deps){
    ...
    this.listen = function(module, listener, role, command){
        this.listeners[module+listener] = {module: module, listener: listener, role: role, command: command};
    }

    this.messageRecieved = function(from, interface, message, user) {
        var self = this;
        if (typeof user === 'undefined') {
            user = from;
        }
        this.prefs.base.findOneAsync({name: 'default_permission'}).done(function(permission){
            this.users.findOneAsync({$and: [{name: user}, {interface: interface}]}).done(function(doc){
    ...
    this.checkRole = function(userRole, actionRole) {
    ...
    this.addYesNoQuestion = function(user, question, yesCallback, noCallback){
    ...
    this.removeLastYesNoQuestion = function(user){
        if (!this.yesNoQuestions[user]) {
            this.yesNoQuestions[user] = [];
        }
    ...
    this.returnLastYesNoQuestion = function(user){
        if (!this.yesNoQuestions[user]) {
            this.yesNoQuestions[user] = [];
        }
    ...
    this.loadCronJobs = function(){
        var self = this;

        this.cron.findAsync({}).done(function(docs){
            if (docs.length > 0) {
                for (var i = 0, len = docs.length; i < len; i++) {
                    new cron(docs[i].crontime, function(){
                        docs[i].callback.call(self.modules.plugins[docs[i].plugin]);
                    })
                }
            }
        })
    ...
    this.newCronJob = function(crontime, callback) {
        var self = this;
        var callback = function(){
            this.turnOn({ip: '192.168.0.4'}, 'shell', null);
        }
        new cron('0 * * * * *', function(){
            callback.call(self.modules.plugins['kankun-plug']);
        }, null, true)
    }
    this.newCronJob();
    return this;
