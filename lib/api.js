...
api = function(deps){
    ...
    this.yesNo = new yesNo();
    this.cron = new cron();
    this.user = new user(deps.users);
};

api.prototype.listen = function(module, listener, role, command, isAlias){
    var listenerKey = module.name + listener.replace(
            /(\((:<(.+?)>)*([\(].+?[\)])*(.+?)\))/g,
...
api.prototype.addListenerAlias = function(module, listener, alias) {
    var registeredListener = this.listeners[module.name + listener],
        listenerParts = registeredListener.listener.match(/\((.+?)\)/g),
        newPartsOrder = [],
        aliasParts = alias.match(/<<(.+?)>>/g);

    listenerParts.forEach(function(value, i) {
        var partName = value.match(/:<(.+?)>/),
            aliasPartNum;
        ...
        aliasPartNum = aliasParts.indexOf('<<' + partName + '>>');

        newPartsOrder[aliasPartNum] = i;

        alias = alias.replace(aliasParts[aliasPartNum], value);
    });
...
api.prototype.messageRecieved = function(interface, from, message, user) {
    ...
    this.user.userFromAccount(user, interface)
        .then(function(doc){
            if(doc){
                return doc.role;
            }
...
api.prototype.loadModule = function(file, type) {
    ...
    return this.prefs[type].findOneAsync({name: modInstance.name}).then(function(doc){
        if (doc && doc.enabled === true) {
            ...
            if (doc.listeneraliases) {
                doc.listeneraliases.forEach(function(listenerAlias) {
                    if (listenerAlias.alias) {
                        self.addListenerAlias(
                            {
                                name: modInstance.name,
                                displayname: modInstance.displayname
                            },
                            listenerAlias.listener,
                            listenerAlias.alias
                        );
                    }
                });
...
api.prototype.reloadModule = function(name, type) {
    ...
    return this.loadModule(fileName, type);
};
