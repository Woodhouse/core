...
api.prototype.messageRecieved = function(interface, from, message, user) {
    ...
    this.users.findOneAsync({$and: [{name: user}, {interface: interface}]}).then(function(doc){
        if(doc){
            return doc.role;
        }

        return self.prefs.interfaces.findOneAsync({name: interface}).then(function(doc) {
            if (doc.default_permission) {
                return doc.default_permission;
            }

            return self.prefs.base.findOneAsync({name: 'default_permission'}).then(function(permission){
...
api.prototype.addBaseClasses = function(module, type){
    ...
    if (type === 'interfaces') {
        module.getPrefs = this.getInterfacePrefs.bind(this, module.name);
    } else if (type === 'plugins') {
        module.getPrefs = this.getPluginPrefs.bind(this, module.name);
    }
}
