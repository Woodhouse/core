        ...
        return this.systemPrefs.get('name').then((instanceName) => {
            ...
            for (let listenerObjectKey in this.listeners) {
                ...
                if (match && this.checkRole(role, this.listeners[listenerObjectKey].role)) {
                    ...
                    this.sendMessage(from, interfaceName, `Matched listener '${this.listeners[listenerObjectKey].listener}' but you don't have the correct permission level. Listener requires '${this.listeners[listenerObjectKey].role}' and you have '${role}'`);
                }
            }

            if (!matched) {
                this.sendMessage(from, interfaceName, `No command matched '${message}'`);
            }
        });
    }
