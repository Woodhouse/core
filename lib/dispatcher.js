...
class dispatcher {
    ...
    listen(module, {id, listener, role, command}) {
        this.listeners[`${module.name}${id}`] = {
            module,
            id,
            listener,
            role,
    ...
    loadListenerAliases(module) {
        return this.moduleData.get('plugin', module.name, 'listeneraliases').then((aliases) => {
            for (const aliasObject of aliases) {
                const registeredListener = this.listeners[`${module.name}${aliasObject.listenerID}`];

                if (registeredListener) {
                    this.listen(
                        module,
                        {
                            id: `${aliasObject.listenerID}_alias`,
                            listener: aliasObject.alias,
                            role: registeredListener.role,
                            command: registeredListener.command
                        }
                    );
                }
            }
