...
class coreListeners {
    ...
    loadConfigListeners() {
        ...
        this.dispatcher.listen(
             {name: 'config', displayname: 'Config'},
            'config list (preferences|prefs) (plugin|interface) (:<plugin or interface name>.+?)',
            'admin',
            (from, interfaceName, message, user) => {
                this.moduleData.getAllPrefs(message[1], message[2]).then((prefs) => {
                    let outputString = '';
        ...
        this.dispatcher.listen(
             {name: 'config', displayname: 'Config'},
            'config set (preference|pref) (:<preference name>.+?) (:<value>.+?) (plugin|interface) (:<plugin or interface name>.+?)',
            'admin',
            (from, interfaceName, message, user) => {
                this.moduleData.setPref(message[3], message[4], message[1], message[2]).then((prefs) => {
                    this.dispatcher.sendMessage(from, interfaceName, `Preference ${message[1]} set to ${message[2]} for ${message[3]} ${message[4]}`);
                });
            }
        );
    }
