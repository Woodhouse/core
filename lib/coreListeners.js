...
class coreListeners {
    ...
    loadHelpListeners() {
        this.dispatcher.listen(
            ...
            {
                id: `help`,
                listener: `help(:<section>.*?)`,
                role: `ignore`,
                command: (from, interfaceName, message, interfaceUser) => {
                    return this.users.userFromAccount(interfaceUser, interfaceName).then((user) => {
                        const groups = {};

                        for (let key in this.dispatcher.listeners) {
                            const listener = this.dispatcher.listeners[key];

                            if (this.dispatcher.checkRole(user.role, listener.role)) {
                                if (!groups[module.displayname]) {
                                    groups[module.displayname] = [];
                                }

                                groups[module.displayname].push({
