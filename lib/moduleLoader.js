...
class moduleLoader {
    constructor(dispatcher, modulePrefs, cron) {
    ...
    loadModule(module, type) {
        ...
        return this.modulePrefs.isEnabled(type, moduleInstance.name).then((enabled) => {
            if (enabled === true) {
                ...
                moduleInstance.init();
                this.dispatcher.loadListenerAliases(moduleInstance.name, type);
            ...
            if (err.shortCode === 'no-prefs') {
                return this.modulePrefs.addModule(moduleInstance, type).then(() => {
                    return false;
                });
            } else {
    ...
    addCoreMethods(module, type) {
        ...
        module.getPref = this.modulePrefs.get.bind(this.modulePrefs, type, module.name);
    }
